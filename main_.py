...
if __name__ == '__main__':
    ...
    for data_type in data_types:
        if data_type == DataType.NonIID:
            num_cluster_list = ["known_labels", 1, 5, 2, 3, 4]
        else:
            ...
            if num_cluster == "known_labels" and data_type == DataType.IID:
                ...
                for net_type in nets_types_list:
                    ...
                    for server_input_tech in server_input_tech_list:
                        ...
                        data_to_pickle[num_cluster][net_type.name][server_input_tech.name] = {}

                        for cluster_technique in cluster_technique_list:
                            experiment_config.cluster_technique = cluster_technique
                            data_to_pickle[num_cluster][net_type.name][server_input_tech.name][cluster_technique.name] = {}
                            for server_feedback_technique in server_feedback_technique_list:
                                ...
                                for t in range(experiment_config.iterations):
                                    ...
                                    data_to_pickle[num_cluster][net_type.name][server_input_tech.name][cluster_technique.name][server_feedback_technique.name]  = rd
                                    pik_name = net_type.name+data_type.name+ exp_type.name+"_"+str(num_cluster)+"_"+cluster_technique.name+"_"+server_feedback_technique.name+"_"+server_input_tech.name
