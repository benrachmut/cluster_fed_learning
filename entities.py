...
class Client(LearningEntity):
    ...
    def fine_tune(self):
        ...
        # Training loop
        for epoch in range(epochs_num_input):
            print(f"Starting epoch {epoch + 1} of {epochs_num_input}.")
            self.epoch_count += 1
            epoch_loss = 0
            for batch_idx, (inputs, targets) in enumerate(fine_tune_loader):
                ...
                # Compute loss
                loss = criterion(outputs, targets)
                # Debugging: Print loss value for the batch
                # Backward pass and optimization
                loss.backward()
                optimizer.step()
                epoch_loss += loss.item()

            # Debugging: Print average loss for the epoch
            avg_loss = epoch_loss / len(fine_tune_loader)
            print(f"Epoch [{epoch + 1}/{epochs_num_input}], Average Loss: {avg_loss:.4f}")

        # Update weights
        self.weights = self.model.state_dict()

        # Return average loss for the last epoch
