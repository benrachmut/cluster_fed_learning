...
class Server(LearningEntity):
    ...
    def train_with_consistency(self, mean_pseudo_labels, cluster_num="0", selected_model=None):
        ...
        data = getattr(self, "eval_global_data", self.global_data)  # same view as PLs
        loader = DataLoader(data, batch_size=experiment_config.batch_size,
                            shuffle=False, num_workers=0, drop_last=False)

        m = self.model if selected_model is None else selected_model
        m.train()
        T = getattr(experiment_config, "T_server", 3.0)
        lam = getattr(experiment_config, "lambda_consistency_server",
        ...
        # class-balancing weights from PLs
        with torch.no_grad():
            q = pseudo_targets_all.clamp_min(1e-8).mean(dim=0)
            cls_w = (1.0 / (q + 1e-8))
        ...
        tau = getattr(experiment_config, "server_conf_threshold", 0.7)
        eps = getattr(experiment_config, "server_label_smooth", 0.05)
        ...
        for ep in range(experiment_config.epochs_num_train_server):
            ...
            for batch_idx, (inputs, _) in enumerate(loader):
                ...
                pseudo_targets = self._normalize_probs(pseudo_targets)
                if eps and eps > 0:
                    pseudo_targets = (1.0 - eps) * pseudo_targets + eps / C

                conf   = pseudo_targets.max(dim=1).values
                w_conf = torch.clamp((conf - tau) / (1.0 - tau + 1e-8), min=0.0, max=1.0)
                ...
                loss_kd = (per * w).sum() / (w.sum() + 1e-8) if w.sum() > 0 else per.mean()
                ...
                if batch_idx == 0:
                    kept = (w_conf > 0).float().mean().item() * 100.0
                    print(f"[Server KD] kept {kept:.1f}% samples (tau={tau})")
    ...
    def train(self, mean_pseudo_labels, cluster_num="0", selected_model=None):
        ...
        for ep in range(experiment_config.epochs_num_train_server):
            ...
            for batch_idx, (inputs, _) in enumerate(loader):
                ...
                per = F.kl_div(logp, pseudo_targets, reduction='none').sum(dim=1) * (T * T)
                loss = (per * w).sum() / (w.sum() + 1e-8) if w.sum() > 0 else per.mean()
